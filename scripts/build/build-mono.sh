...
if [ ! -z "$OFFLINE" ]; then
   ...
   $REPOROOT/scripts/obtain/install-tools-mono.sh
    
   $REPOROOT/scripts/build/restore-packages-mono.sh
fi

header "Compiling"
$REPOROOT/scripts/compile/compile-mono.sh

header "Setting Stage2 as PATH, DOTNET_HOME, and DOTNET_TOOLS"
export DOTNET_HOME=$STAGE2_DIR
export DOTNET_TOOLS=$STAGE2DIR
